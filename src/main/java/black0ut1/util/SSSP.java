...
public class SSSP {
	...
	public static Pair<Network.Edge[], double[]> minTree(Network network, int root, double[] costs) {
	...
	public static Pair<Network.Edge[], double[]> minTree(Network network, int root) {
		double[] distance = new double[network.nodes];
		Arrays.fill(distance, Double.POSITIVE_INFINITY);
		distance[root] = 0;
		...
		while (!pq.isEmpty()) {
			...
			for (Network.Edge edge : network.neighborsOf(fromVertex)) {
				...
				double newDistance = distance[fromVertex] + edge.freeFlow;
				if (mark[toVertex] == 0) {
					mark[toVertex] = 1;
					distance[toVertex] = newDistance;
					previous[toVertex] = edge;
	...
	public static Pair<Network.Edge[], int[]> minTreeLen(Network network, int root, double[] costs) {
		double[] distance = new double[network.nodes];
		Arrays.fill(distance, Double.POSITIVE_INFINITY);
		distance[root] = 0;
	...
	public static Pair<Network.Edge[], int[]> minTreeLen(Network network, int root) {
		double[] distance = new double[network.nodes];
		Arrays.fill(distance, Double.POSITIVE_INFINITY);
		distance[root] = 0;
